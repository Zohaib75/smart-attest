...
async function Request_IBCC(tx) {
    ...
    const result = await request.get({ uri: 'http://localhost:2001/'+tx.cnic, json: true });

    if(result.length > 0){

        let concept = factory.newConcept(namespace, 'IBCC_Concept');
        concept.matric = result.matric
        concept.inter = result.inter
...
async function Request_Nadra(tx) {
    ...
    const result = await request.get({ uri: 'http://localhost:2000/'+tx.cnic, json: true });

    if(result.length > 0){

        let concept = factory.newConcept(namespace, 'NADRA_Concept');
        concept.name = result.name
        concept.Father_name = result.Father_name
        concept.Address = result.Address
        concept.email = result.email
        concept.mobile = result.mobile
        concept.city = result.city        
        return concept;
...
async function Add_Student(tx) {
    ...
    const std = factory.newResource(namespace, 'Student', tx.StudentId);
    std.cnic = tx.cnic;
...
async function Add_Department(tx) {
    ...
    tx.DepartmentId = tx.DepartmentId + tx.university.getIdentifier()
